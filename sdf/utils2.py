...
class Trainer(object):
    def __init__(self, 
        ...
        if B1!=None:
            self.B1 = B1
            self.B1_optimizer = w_optimizer(self.B1)
        else:
            self.B1 = None
            self.B1_optimizer = None
        if B2!=None:
            self.B2 = B2
            self.B2_optimizer = w_optimizer(self.B2)
        else:
            self.B2 = None
        ...
        self.W_optimizer = w_optimizer(self.W)  if  self.W!=None else None
        self.N_optimizer = w_optimizer(self.N) if  self.N!=None else None
        self.M_optimizer = w_optimizer(self.M)  if  self.M!=None else None
        ...
        if lr_scheduler is None:
                self.W_lr_scheduler = optim.lr_scheduler.LambdaLR(self.W_optimizer, lr_lambda=lambda epoch: 1) if self.W_optimizer!=None else None # fake scheduler
                self.N_lr_scheduler = optim.lr_scheduler.LambdaLR(self.N_optimizer, lr_lambda=lambda epoch: 1) if self.N_optimizer!=None else None # fake scheduler
                self.M_lr_scheduler = optim.lr_scheduler.LambdaLR(self.M_optimizer, lr_lambda=lambda epoch: 1) if self.N_optimizer!=None else None
                if B1!=None:
                    self.B1_lr_scheduler = optim.lr_scheduler.LambdaLR(self.B1_optimizer, lr_lambda=lambda epoch: 1)
                if B2!=None:
                    self.B2_lr_scheduler = optim.lr_scheduler.LambdaLR(self.B2_optimizer, lr_lambda=lambda epoch: 1)
        else:
                self.W_lr_scheduler = lr_scheduler(self.W_optimizer) if self.W_optimizer!=None else None
                self.N_lr_scheduler = lr_scheduler(self.N_optimizer) if self.N_optimizer!=None else None
                self.M_lr_scheduler = lr_scheduler(self.M_optimizer) if self.M_optimizer!=None else None
                if B1!=None:
                    self.B1_lr_scheduler = optim.lr_scheduler.LambdaLR(self.B1_optimizer, lr_lambda=lambda epoch: 1)
                if B2!=None:
                    self.B2_lr_scheduler = optim.lr_scheduler.LambdaLR(self.B2_optimizer, lr_lambda=lambda epoch: 1)
        ...
        if ema_decay is not None:
                ema = ExponentialMovingAverage([self.W], decay=ema_decay) if self.W!=None else None
                self.W_ema=ema
                
                ema = ExponentialMovingAverage([self.N], decay=ema_decay) if self.N!=None else None
                self.N_ema=ema
                
                ema = ExponentialMovingAverage([self.M], decay=ema_decay) if self.M!=None else None
                self.M_ema=ema
                
                if B1!= None:
                    ema = ExponentialMovingAverage([self.B1], decay=ema_decay)
                    self.B1_ema=ema
                else:
                    self.B1_ema = None
                if B2!= None:
                    ema = ExponentialMovingAverage([self.B2], decay=ema_decay)
                    self.B2_ema=ema
                else: 
                    self.B2_ema = None
        else:
            ...
            if B1!= None:
                    ...
                    self.B1_ema=None
            if B2!= None:
                    # ema = Exponential/MovingAverage([self.B2], decay=ema_decay)
                    self.B2_ema=None
    ...
    def w_train_step(self,mode, data, model,W=None,N=None,M=None,B1=None,B2=None):
        # assert batch_size == 1
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                if epoch%5==0:
                    for idx,model in enumerate(self.models):
                            torch.save(model.encoder,self.workspace+f'/GT_enc_{idx}.pth')
                            torch.save(model.backbone,self.workspace+f'/GT_mlp.pth')
                            print("Completed saving path of models")
                torch.cuda.empty_cache()
                # torch.save(model.encoder,self.workspace+f'/GT_enc.pth')
                # torch.save(model.backbone,self.workspace+f'/GT_mlp.pth')
                # print("Completed saving path of models")
    ...
    def w_train(self, mode,multiple_rots,train_loaders, valid_loaders, max_epochs,macro_batch_size):
        if self.use_tensorboardX and self.local_rank == 0:
            self.writer = tensorboardX.SummaryWriter(os.path.join(self.workspace, "run", self.name))
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                self.w_train_one_epoch(mode,multiple_rots,train_loaders,self.models,macro_batch_size)
                if epoch%2==0:
                    if mode == 'lora':
                        torch.save(self.N,self.workspace+f'N.pth')
                        torch.save(self.M,self.workspace+f'M.pth')
                        if self.B1 !=None:
                            torch.save(self.B1,self.workspace+f'B1.pth')
                        if self.B2 !=None:
                            torch.save(self.B2,self.workspace+f'B2.pth')
                        # for idx,model in enumerate(self.models):
                        ...
                        torch.save(self.W,self.workspace+f'W.pth')
                    print("Completed saving path of models")
                torch.cuda.empty_cache()
    ...
    def w_forward_epoch_gradaccum(self,mode,subset_loaders_iter, subset_models, div_factor):
        global total_loss
        global loader_epoch_loss
        for j, loader_iter in enumerate(subset_loaders_iter):
                data = next(loader_iter)
                ...
                data = self.prepare_data(data)
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, truths, loss = self.w_train_step(mode,data,subset_models[j],self.W,self.N,self.M,self.B1,self.B2)
    ...
    def multiple_w_forward_epoch_gradaccum(self,mode,subset_loaders_iter, subset_models, div_factor):
        global total_loss
        global loader_epoch_loss
        w = self.W
        for j, loader_iter in enumerate(subset_loaders_iter):
                ...
                data = self.prepare_data(data)
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, truths, loss = self.w_train_step(mode,data,subset_models[j],w,self.N,self.M,self.B1,self.B2)
                
                if j == 0:
                    t_loss = loss/div_factor
                else:
                    t_loss += loss/div_factor
                
                loss_val = loss.item()
                w = torch.matmul(w,self.W)
    ...
    def w_train_one_epoch(self, mode, multiple_rots,loaders, models, macro_batch_size):
        if mode == 'lora':
            self.log(f"==> Start Training Epoch {self.epoch}, lr={self.N_optimizer.param_groups[0]['lr']:.6} ...")
        elif mode == 'normal':
            self.log(f"==> Start Training Epoch {self.epoch}, lr={self.W_optimizer.param_groups[0]['lr']:.6} ...")
        global total_loss
        total_loss = 0
        ...
        for i in range(N):
            ...
            if self.N_optimizer is not None:
                self.N_optimizer.zero_grad()
            if self.N_optimizer is not None:
                self.M_optimizer.zero_grad()
            if self.B1_optimizer!=None:
                self.B1_optimizer.zero_grad()
            if self.B2_optimizer!=None:
                self.B2_optimizer.zero_grad()
                
            subset_n = macro_batch_size
            if multiple_rots: 
                for i in range(0, len(loaders_iter), subset_n):
                    subset_models = [models[0] for i in range(subset_n)]
                    subset_loaders_iter = loaders_iter[i:i + subset_n]
                    self.multiple_w_forward_epoch_gradaccum(mode,subset_loaders_iter, subset_models,div_factor)
                    # w = torch.matmul(w,self.W)
            else:
                for i in range(0, len(models), subset_n):
                    subset_models = models[i:i + subset_n]
                    subset_loaders_iter = loaders_iter[i:i + subset_n]
                    self.w_forward_epoch_gradaccum(mode,subset_loaders_iter, subset_models,div_factor)
                
            # for enc_opt in self.enc_optimizers:
            #     self.scaler.step(enc_opt)
            # self.scaler.step(self.net_optimizer)
            if mode == 'lora':
                self.scaler.step(self.N_optimizer)
                self.scaler.step(self.M_optimizer)
                if self.B1_optimizer!=None:
                    self.scaler.step(self.B1_optimizer)
                if self.B2_optimizer!=None:
                    self.scaler.step(self.B2_optimizer)
            elif mode == 'normal':
            ...
            if mode == 'normal':
                if self.W_ema is not None:
                        self.W_ema.update()
            if mode == 'lora':
                if self.N_ema is not None:
                        self.N_ema.update()
                if self.M_ema is not None:
                        self.M_ema.update()
                        
                if self.B1_ema is not None:
                        self.B1_ema.update()
                if self.B2_ema is not None:
                        self.B2_ema.update()
        ...
        if self.scheduler_update_every_step:
                ...
                # self.net_lr_scheduler.step()
                if mode == 'lora':
                    self.N_lr_scheduler.step()
                    self.M_lr_scheduler.step()
                    if self.B1_lr_scheduler is not None: 
                        self.B1_lr_scheduler.step()
                    if self.B2_lr_scheduler is not None: 
                        self.B2_lr_scheduler.step()
                elif mode == 'normal':
                    self.w_lr_scheduler.step()
        ...
        #         # [lr_scheduler.step(average_loss) for lr_scheduler in self.enc_lr_schedulers]
        #         # self.net_lr_scheduler.step(average_loss)
        #         self.N_lr_scheduler.step(average_loss)
        #         self.M_lr_scheduler.step(average_loss)
        #     else:
        #         # [lr_scheduler.step(average_loss) for lr_scheduler in self.enc_lr_schedulers]
        #         # self.net_lr_scheduler.step()
        #         self.N_lr_scheduler.step()
