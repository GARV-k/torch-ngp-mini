...
if __name__ == '__main__':
    ...
    valid_loaders = []
    # for i in range(_n):
    for i in range(obj_no,_n+obj_no):
    ...
    # for i in range(_n):
    for i in range(obj_no,_n+obj_no):  
        valid_dataset = SDFDataset(obj_path_pre+f'{i}.obj', size=100, num_samples=2**18,sample_ratio_n=n,sample_ratio_d=d) # just a dummy
    ...
    N_optimizer = lambda w: torch.optim.Adam([
        {'name': 'encoding', 'params': [N]}
        #{'name': 'net', 'params': model.backbone.parameters(), 'weight_decay': 1e-6},      
    ], lr=lr, betas=(0.9, 0.99), eps=1e-15)
    ...
    scheduler = lambda optimizer: optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1)
    subset_n = 1
    trainer = Trainer('ngp', _n, models,workspace=workspace1, enc_optimizer=enc_optimizer,  net_optimizer=net_optimizer, N_optimizer = N_optimizer,M_optimizer = M_optimizer, criterion=criterion, ema_decay=0.95, fp16=False, lr_scheduler=scheduler, use_checkpoint='latest', eval_interval=1,scheduler_update_every_step=False)
    trainer.train(train_loaders, valid_loaders,1000,subset_n)
