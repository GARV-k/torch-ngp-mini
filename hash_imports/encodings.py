...
from hash_imports.base_field_component import FieldComponent
from hash_imports.external import TCNN_EXISTS, tcnn
from hash_imports.math_imports import components_from_spherical_harmonics, expected_sin, generate_polyhedron_basis
from hash_imports.printing import print_tcnn_speed_warning

def batch_mat_mul(w, hash_table, division_factor=2):
    # Ensure w is on CPU and hash_table is on CUDA
    assert not w.is_cuda, "Input matrix w should be on CPU"
    assert hash_table.is_cuda, "Hash table should be in CUDA memory"
    
    # Get the dimensions of w and hash_table
    n, d = hash_table.size()  # hash_table: (n, d)
    assert w.size(0) == n and w.size(1) == n, "w must have dimensions (n, n)"
    
    # Initial chunk size
    chunk_size = (n + division_factor - 1) // division_factor  # ceil(n / division_factor)
    ...
    while i < n:
        try:
            # Adjust the chunk size dynamically if needed
            w_chunk = w[i:i+chunk_size, :]  # Select the next chunk (t, n) where t <= chunk_size
            
            # Transfer the chunk to CUDA
            w_chunk_cuda = w_chunk.to('cuda')

            # Perform matrix multiplication with the hash table on CUDA
            with torch.no_grad():
                result_chunk_cuda = torch.matmul(w_chunk_cuda, hash_table)
            
            # Transfer the result back to CPU
            result_chunk_cpu = result_chunk_cuda.to('cpu')
            
            # Append the result to the output list
            output_list.append(result_chunk_cpu)
            
            # Free up GPU memory after each chunk
            del w_chunk_cuda, result_chunk_cuda
            ...
            if 'out of memory' in str(e):
                # If we run out of memory, reduce the chunk size and retry
                torch.cuda.empty_cache()
                chunk_size = max(1, chunk_size // 2)  # Reduce chunk size by half
                print(f"Out of memory, reducing chunk size to {chunk_size}")
            else:
                # Raise any other errors
                raise e

    # Concatenate all chunks to get the final result
    final_output = torch.cat(output_list, dim=0)
    
    # Ensure the first dimension of the final output is n
    assert final_output.size(0) == n, "Final output's first dimension must be n"
    
    return final_output.to('cuda')

def modified_matmul(n, m, hash_table, num_parts):
    # Get the dimensions of the input matrices
    N, f = n.shape
    f2, M = m.shape
    
    # Ensure the dimensions are compatible for matrix multiplication
    assert f == f2, "Inner dimensions must match for matrix multiplication."
    
    # Initialize an empty list to hold the results
    results = []
    
    # Calculate the interval for partitioning
    interval = M // num_parts + (M % num_parts > 0)  # Ceiling division
    
    # Iterate over the number of parts
    for i in range(0, M, interval):
        # Slice the transform matrix
        m_slice = m[:, i:i + interval].contiguous()

        # Perform the matrix multiplication
        U = torch.matmul(n, m_slice)
        new_hash_table = torch.matmul(hash_table.T, U).T

        # Store the result in the list
        results.append(new_hash_table)
        
        # Clean up unnecessary variables
        del U, new_hash_table, m_slice
        
        # Clear cache to free up GPU memory (only cached memory, not allocated)
        torch.cuda.empty_cache()
...
class HashEncoding(Encoding):
    ...
    def transform_custom(self, mode,W=None,N=None,M=None,B1=None,B2=None):
        """
        Size: hash_table (self.hash_table_size * self.num_levels, self.features_per_level)

        transform_matrix: (self.hash_table_size * self.num_levels, self.hash_table_size * self.num_levels)
        """

        # new_hash_table = torch.matmul(self.hash_table.T, transform_matrix).T
        # new_hash_table = modified_matmul(N,M,self.hash_table,100)
        new_hash_table = None
        if mode == 'lora':
            if (N is not None) and (M is not None):
                if B1== None:
                    new_hash_table1 = torch.matmul(self.hash_table.T, N)
                else :
                    new_hash_table1 = torch.matmul(self.hash_table.T, N) + B1
                if B2== None:
                    new_hash_table = torch.matmul(new_hash_table1,M).T
                
                else:
                    new_hash_table = torch.matmul(new_hash_table1,M).T + B2
                    # new_hash_table = batch_mat_mul(transform_matrix,self.hash_table,4)
            else: 
                print("ERROR : N or M is None.")
        elif mode=='normal':
            
            if W is not None:
                new_hash_table = torch.matmul(self.hash_table.T, W).T
            else:
                print('ERROR : W is None.')
            
        if new_hash_table == None:
            print('ERROR : New hash Table is None')
        return new_hash_table
        
    def pytorch_fwd(self, in_tensor: Float[Tensor, "*bs input_dim"],mode = 'normal',W = None, N=None,M=None,B1 = None,B2 = None) -> Float[Tensor, "*bs output_dim"]:
        ...
        # if w != None:
        #     new_hash_table = self.transform_custom(w)
        ...
        new_hash_table = self.transform_custom(mode,W,N,M,B1,B2)
        # torch.save(new_hash_table,'new_hash_table.pth')
        # print('saving new_hash_table')
        ...
        if (N == None or M == None) and (W is None) :
            f_0 = self.hash_table[hashed_0]  # [..., num_levels, features_per_level]
            f_1 = self.hash_table[hashed_1]
            f_2 = self.hash_table[hashed_2]
            f_3 = self.hash_table[hashed_3]
            ...
            f_0 = new_hash_table[hashed_0]  # [..., num_levels, features_per_level]
            f_1 = new_hash_table[hashed_1]
            f_2 = new_hash_table[hashed_2]
            f_3 = new_hash_table[hashed_3]
            f_4 = new_hash_table[hashed_4]
            f_5 = new_hash_table[hashed_5]
            f_6 = new_hash_table[hashed_6]
            f_7 = new_hash_table[hashed_7]
    ...
    def forward(self, in_tensor: Float[Tensor, "*bs input_dim"],mode='normal',W=None,N=None,M = None,B1=None,B2=None) -> Float[Tensor, "*bs output_dim"]:
        if self.tcnn_encoding is not None:
            return self.tcnn_encoding(in_tensor)
        return self.pytorch_fwd(in_tensor,mode,W,N,M,B1,B2)
